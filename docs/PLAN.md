...
- [x] `/` n'affiche **que les offres de la dernière exécution réussie**, statut « à traiter », intérêt ≥ ~~50~~ ~~35~~ **40**, classées par intérêt décroissant. ⚠️ **Le seuil a été abaissé par Maxime le 30 août 2026** (50 → 35), après mesure : à 50 l'écran était vide quatre matins sur six. ⚠️ **Puis remonté à 40 le 31 août 2026, et surtout ÉTENDU À `/offres`** : il ne borne plus le seul écran du matin. Il a donc quitté `matin.ts` pour `interface/lib/filtres.ts` (`SEUIL_INTERET`) — écran réel : « 1 offre retenue sur 7 collectées », l'offre à 68 s'affiche, les six autres (45, 35, 30…) non
